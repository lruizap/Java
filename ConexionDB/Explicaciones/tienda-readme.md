...
2. **Interactuar con la Base de Datos**:
   El método `interactuarConBaseDeDatos` permite al usuario seleccionar diferentes opciones para interactuar con la base de datos.
...
4. **Ver Datos de una Tabla**:
   Muestra todos los datos de una tabla especificada.
...
5. **Modificar Datos**:
   Permite modificar datos de una tabla especificada.
...
6. **Crear Nuevo Registro**:
   Permite crear un nuevo registro en las tablas `usuarios`, `productos` o `pedidos`.
...
7. **Eliminar Registro**:
   Permite eliminar un registro de una tabla especificada.
...
2. Asegúrate de que tu servidor MySQL esté en ejecución y que los parámetros de conexión en el archivo `App.java` sean correctos.
3. Compila y ejecuta el programa desde Visual Studio Code.
4. Sigue las instrucciones en la consola para interactuar con la base de datos.
